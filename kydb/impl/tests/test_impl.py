...
ALL_DB_TYPES = ['memory', 'files'] if is_automated_test() else ['memory', 's3', 'redis', 'dynamodb', 'files']
# ALL_DB_TYPES = ['memory']
...
DB_URLS = {
    ...
    'redis': 'redis://{}:6379'.format(os.environ.get('KINYU_UNITTEST_REDIS_HOST')),
...
@pytest.mark.parametrize('db_type', (x for x in ALL_DB_TYPES if x != 'memory'))
